#include <stdio.h>
#include <stdlib.h>
/* Honestly have no clue how this works, I just adapted Alistair's formatting
	and added one more to RATE, RATEM, THRESH, BASE and added the corresponding
	if statements. I'll have a good look at this later. */
#define RATE0   0.000
#define RATE1   0.190
#define RATE2   0.325
#define RATE3   0.370
...
main(int argc, char *argv[]) {
	...
	if (gross <= THRESH1) {
		...
		tax = BASE5 + (gross-THRESH4)*RATE5;
	}
	if (gross <= 100000)
		medicare = RATEM0*gross;
	else medicare = RATEM1*gross;
